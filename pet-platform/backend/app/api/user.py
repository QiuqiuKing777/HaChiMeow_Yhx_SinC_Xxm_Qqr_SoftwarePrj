...
@jwt_required()
def get_public_user_old(user_id):
    user = User.query.get_or_404(user_id)
...
def create_address():
    ...
    if is_default:
        UserAddress.query.filter_by(user_id=user_id).update({
            'is_default': False
        })
...
@user_bp.route('/user/addresses/<int:address_id>', methods=['PUT'])
@jwt_required()
...
@user_bp.route('/user/addresses/<int:address_id>', methods=['DELETE'])
@jwt_required()
...
def list_favorites():
    ...
    target_type = (
        request.args.get('target_type', '').strip()
        or request.args.get('type', '').strip()
    )
...
@user_bp.route('/user/favorites/<string:target_type>/<int:target_id>', methods=['DELETE'])
@jwt_required()
...
@user_bp.route('/user/favorites/<int:favorite_id>', methods=['DELETE'])
@jwt_required()
...
@user_bp.route('/user/notifications/read-all', methods=['PUT'])
@jwt_required()
def read_all():
    user_id = current_user_id()
