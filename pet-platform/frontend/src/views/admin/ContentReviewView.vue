    ...
    <div class="page-card">
      <el-tabs v-model="activeTab" @tab-change="onTabChange" class="admin-tabs">
        <el-tab-pane label="宠物审核" name="pets">
          ...
          <el-table :data="pets.list" v-loading="pets.loading" stripe class="admin-table">
            <el-table-column prop="pet_id" label="ID" width="70" />
            <el-table-column label="头像" width="80">
              <template #default="{ row }">
                <el-avatar :size="40" :src="row.cover_image || '/NKU.png'" shape="square" style="background: #e6a23c">宠</el-avatar>
              </template>
            </el-table-column>
            <el-table-column prop="pet_name" label="名称" min-width="150" />
            <el-table-column prop="species" label="种类" width="90" />
            <el-table-column prop="age_desc" label="年龄" width="90" />
            <el-table-column label="发布方" width="120">
              <template #default="{ row }">{{ row.publisher?.nickname || row.publisher?.username || '-' }}</template>
            </el-table-column>
          ...
          <el-pagination
            ...
            :total="pets.total"
            :page-size="15"
            v-model:current-page="pets.page"
        ...
        <el-tab-pane label="商品审核" name="products">
          ...
          <el-table :data="products.list" v-loading="products.loading" stripe class="admin-table">
            <el-table-column prop="product_id" label="ID" width="70" />
            <el-table-column label="图片" width="80">
              <template #default="{ row }">
                <el-avatar :size="40" :src="row.cover_image || '/NKU.png'" shape="square" style="background: #67c23a">品</el-avatar>
              </template>
            </el-table-column>
            ...
            <el-table-column label="状态" width="100">
              <template #default="{ row }">
                <el-tag :type="genericStatusType(row.status)">
        ...
        <el-tab-pane label="服务审核" name="services">
          ...
          <el-table :data="services.list" v-loading="services.loading" stripe class="admin-table">
          ...
          <el-pagination
            ...
            :page-size="15"
            v-model:current-page="services.page"
...
const statusType = (status) => ({ online: 'success', pending: 'warning', offline: 'info', adopted: '' })[status] || ''
const genericStatusType = (status) => ({ online: 'success', pending: 'warning', offline: 'info' })[status] || ''
const petStatusLabel = (status) => ({ online: '可领养', pending: '待审核', offline: '已下线', adopted: '已领养' })[status] || status
